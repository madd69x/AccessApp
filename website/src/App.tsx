...
const DownloadButton = ({ large = false }: { large?: boolean }) => (
  <a
    ...
    target="_blank"
    rel="noreferrer"
    className={`group relative inline-flex items-center justify-center overflow-hidden rounded-2xl bg-white text-black font-semibold transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(255,255,255,0.12)] ${
      large ? 'px-10 py-5 text-[15px] gap-3' : 'px-8 py-4 text-[13px] gap-2.5'
    }`}
  >
    <span className="absolute inset-0 bg-gradient-to-r from-transparent via-black/5 to-transparent translate-x-[-200%] group-hover:translate-x-[200%] transition-transform duration-700" />
    <Download size={large ? 18 : 15} strokeWidth={2} className="relative z-10" />
    <span className="relative z-10">Download APK</span>
    <ArrowRight size={large ? 16 : 14} className="relative z-10 opacity-0 -ml-2 group-hover:opacity-100 group-hover:ml-0 transition-all duration-300" />
  </a>
);
...
const GithubButton = ({ large = false }: { large?: boolean }) => (
  <a
    ...
    target="_blank"
    rel="noreferrer"
    className={`group relative inline-flex items-center justify-center overflow-hidden rounded-2xl bg-[#0A0A0A] border border-[#222] text-[#AAA] font-semibold transition-all duration-300 hover:-translate-y-1 hover:text-white hover:border-[#444] hover:shadow-[0_8px_30px_rgba(255,255,255,0.04)] ${
      large ? 'px-10 py-5 text-[15px] gap-3' : 'px-8 py-4 text-[13px] gap-2.5'
    }`}
  >
    <svg className="relative z-10 flex-shrink-0" style={{width: large ? '18px' : '15px', height: large ? '18px' : '15px'}} viewBox="0 0 24 24" fill="currentColor">
